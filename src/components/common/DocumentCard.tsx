...
const DocumentCard = ({ title, createdDate, updatedDate }: Props) => {
  return (
    <div className='group w-full min-h-[16rem] p-2.5 border border-gray-200 rounded-lg flex flex-col justify-around transition-all duration-200 ease-in-out hover:shadow-[0_3px_10px_rgb(0,0,0,0.2)]'>
      <div className='relative bg-blue-50 w-full min-h-[10rem] rounded-md flex justify-center items-center'>
        ...
        <LuFileText className='text-8xl text-blue-600' />
      </div>
      <div className='flex flex-col justify-center'>
        <p className='text-xl capitalize font-medium'>{title}</p>
        <div className='flex justify-between'>
          <p className='text-sm text-gray-500'>Created: {createdDate}</p>
          <p className='text-sm text-gray-500'>Updated: {updatedDate}</p>
        </div>
      </div>
    </div>
  );
